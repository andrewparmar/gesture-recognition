...
def kf_matching_sensor(frame, noise, template):
    corr_map = cv2.matchTemplate(frame.astype(np.uint8), template, cv2.TM_SQDIFF)
...
def get_centers_distance(pt_1, pt_2):
    return np.sqrt(
        (float(pt_1[0]) - float(pt_2[0])) ** 2 + (float(pt_1[1]) - float(pt_2[1])) ** 2
    )
...
class PS5_PF_Tests(unittest.TestCase):
    ...
    def check_distance(
        ...
        self.assertTrue(
            ...
            "Student's center: {} \n"
            "Actual center: {} \n"
            "Max euclidean distance allowed: {}".format(
                frame_num, predicted_center, actual_center, max_distance
            ),
        )
